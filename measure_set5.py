...
image_name_list = ['baby', 'bird', 'butterfly', 'head', 'woman']
...
for image_name in image_name_list:
    gt = os.path.expanduser(os.path.join(gt_dir, f'{image_name}.png'))
    ...
    if n_samples > 1:
        ...
        for i in range(n_samples):
            sr = os.path.join(srs_dir, f'{image_name}_sample{i:02d}.png')  # FIXME
        ...
        srs_imgs.append([])
        sr = os.path.join(srs_dir, f'{image_name}_1000001.png')
        if sr in srs_imgs_raw:
            srs_imgs[-1].append(sr)
        else:
            raise RuntimeError("Not Found: ", sr)
